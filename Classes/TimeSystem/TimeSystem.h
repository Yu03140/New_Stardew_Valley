...
#define TIME_SCALE 1.0/10.0     // 时间缩放比例：现实中30s=游戏中1小时
#define DROUGHT_DAY_PROBABILITY 0.1 // 干旱天的概率：10%的概率会生成干旱天
#define SPRING 1
#define SUMMER 2
#define AUTUMN 3
#define WINTER 4
...
private:
    ...
    int day;                // 当前天数
    int hour;               // 当前小时
    int season;             // 当前季节：春1夏2秋3冬4
    std::string weather;        // 天气
    //bool hasEnteredChristmasScene = false; // 标志：记录事件是否已经进入圣诞场景


    static TimeSystem* instance;  // 静态成员变量，保证对象唯一实例
